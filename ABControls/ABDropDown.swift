...
public class ABDropDown: ABTextualControl, UITableViewDelegate, UITableViewDataSource {
	...
	private func showList() {
		...
		if _tableview.isHidden {
			frame = CGRect(x: _frame.origin.x,
				y: _frame.origin.y,
				width: _frame.width,
	...
	private func setupLabel() {
		_label = UILabel(frame: bounds.insetBy(dx: 5, dy: 0))
		addSubview(_label)
	...
	private func setupDropdownButton() {
		...
		_button.frame = CGRect(x: bounds.width - 30, y: 2.5, width: 25, height: 25)
		...
		setNeedsDisplay()
		#if !TARGET_INTERFACE_BUILDER
			_button.addTarget(self, action: #selector(showList), for: UIControl.Event.touchUpInside)
		#endif
	}

	private func setupTableviewDropdown() {
		_tableview.frame = CGRect(x: 0, y: _defaultHeight, width: frame.width - 20, height: _dropdownViewHeight)
	...
	public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
		return _listItems != nil ? (_listItems?.count)! : 0
	}
